...
from modules.fakenews.services.application.create_fake_news_service import CreateFakeNewsService
...
class FakeNewsController(APIController):
    def __init__(self):
        ...
        self.create_fake_news_service = CreateFakeNewsService(
            self.image_builder_service,
            self.headline_generator_service
        )
    
    @API.route("/", method=HTTPMethod.POST, response_model=CreateFakeNewsResponseDto)
    async def create_fake_news(
        self,
        name: str = Form(..., min_length=1, max_length=255, description="Nome da pessoa para a notícia falsa"),
        image: UploadFile = File(..., description="Imagem para incluir na notícia falsa")
    ) -> JSONResponse:
        try:
