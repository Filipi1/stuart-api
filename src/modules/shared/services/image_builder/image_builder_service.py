...
class ImageBuilderService:
    ...
    def __init__(self):
        ...
        self.main_content_width = (
            self.sidebar_x_start - self.main_content_x_start - self.margin
        )
    ...
    def create_fake_news_image(
        ...
        try:
            ...
            final_image = self._combine_images(
                frame_image, received_image, headline, subtitle
            )
    ...
    def _create_default_newspaper_frame(self) -> Image.Image:
        ...
        draw.rectangle(
            [0, 0, self.max_width - 1, self.max_height - 1], outline="black", width=2
        )
    ...
    def _draw_sidebar(self, draw: ImageDraw.Draw):
        ...
        draw.line(
            [(sidebar_x, y_pos), (sidebar_x + sidebar_width, y_pos)],
            fill="black",
            width=1,
        )
        y_pos += 15
    ...
    def _combine_images(
        ...
        x_offset = (
            self.main_content_x_start
            + (self.main_content_width - content_image.width) // 2
        )
        y_offset = 220
        ...
        if subtitle:
            title_height = self._calculate_text_height(
                draw, headline, self.title_font_size
            )
    ...
    def _calculate_text_height(
        self, draw: ImageDraw.Draw, text: str, font_size: int
    ) -> int:
    ...
    def _wrap_text(
        self, draw: ImageDraw.Draw, text: str, font: ImageFont.ImageFont, max_width: int
    ) -> list[str]:
        words = text.split()
        lines = []
        current_line = []
    ...
    def _draw_text_lines(
        ...
        for i, line in enumerate(lines):
            ...
            self._draw_single_text_line(
                draw, line, font, y_pos, x_start_position, max_text_width, bold, color
            )

    def _draw_single_text_line(
        self,
        draw: ImageDraw.Draw,
        line: str,
        font: ImageFont.ImageFont,
        ...
        for offset_x in range(-1, 2):
            for offset_y in range(-1, 2):
                if offset_x != 0 or offset_y != 0:
                    draw.text(
                        (x_position + offset_x, y_pos + offset_y),
                        line,
                        fill=color,
                        font=font,
                    )
