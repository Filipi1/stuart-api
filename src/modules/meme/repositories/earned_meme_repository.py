...
class EarnedMemeRepository(RepositoryAdapter):
    ...
    async def increase_meme_to_user(self, user_id: int, meme_id: int) -> EarnedMemeEntity:
        response = await self.__supabase_service.read(self.table, {"userId": user_id, "memeId": meme_id})
        if response and len(response) > 0:
            earned_meme = EarnedMemeEntity(**response[0])
            response = await self.__supabase_service.update(self.table, earned_meme.id, {"earnedTimes": earned_meme.earned_times + 1})
            return EarnedMemeEntity(**response[0])
        response = await self.__supabase_service.create(self.table, {
            "userId": user_id,
            "memeId": meme_id,
            "earnedTimes": 1,
        })
